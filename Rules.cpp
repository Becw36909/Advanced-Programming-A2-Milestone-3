...
bool Rules::validateMoveEnhanced(GameBoard* board, const std::vector<Tile*>& tiles, const std::vector<std::pair<int, int>>& positions) {
  if (tiles.size() != positions.size()) {
        return false; // Number of tiles must match number of positions
    }
    for (size_t i = 0; i < tiles.size(); ++i) {
        if (!isValidPlacement(board, tiles[i], positions[i].first, positions[i].second)) {
